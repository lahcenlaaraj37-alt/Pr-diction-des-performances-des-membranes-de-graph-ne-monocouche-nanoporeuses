...
# ============================================================
# === Responsive Sidebar Toggle System ===
# ============================================================
def sidebar_toggle():
    st.markdown("""
    <style>
    /* ===== Floating menu button ===== */
    div[data-testid="stButton"]:has(button[key="toggle_sidebar_btn"]) {
        position: fixed !important;
        top: 12px !important;
        left: 12px !important;
    ...
    @media (max-width: 768px) {
        ...
        #top-header-responsive div:last-child {
    ...
    function syncSidebarButtons() {
        ...
        const nativeBtn = document.querySelector('button[data-testid="collapsedControl"]');

        const customBtn = Array.from(document.querySelectorAll('button'))
            .find(el => el.innerText.includes('☰ Menu'));

        if(nativeBtn && customBtn){

            nativeBtn.style.zIndex = "999999";
            customBtn.style.zIndex = "1000000";
    ...
    setTimeout(syncSidebarButtons, 1000);
    </script>
    """, unsafe_allow_html=True)

    if st.button("☰ Menu", key="toggle_sidebar_btn"):
        js = """
        <script>
        const sidebar = document.querySelector('section[data-testid="stSidebar"]');
        if(sidebar){
...
def sidebar_toggle():
    ...
    # JavaScript: Surveillance du bouton Streamlit original maintenant en bas
    # Lorsque l'utilisateur clique dessus pour masquer la barre latérale, nous programmons un clic sur le bouton ☰ Menu
    # Pour synchroniser session_state
    st.markdown(
        """
        <script>
        (function () {
...
""", unsafe_allow_html=True)
    ...
    st.markdown(
        """
<div id="top-header-responsive" style="position: fixed; top: 0; left: 0; right: 0; width: 100%; background: linear-gradient(135deg, #1e3a8a 0%, #2c5282 100%); padding: 1.5rem 2rem; padding-left: 350px; z-index: 9999; box-shadow: 0 4px 20px rgba(30, 58, 138, 0.3); display: flex; justify-content: space-between; align-items: center;">
