...
# ============================================================
# === CSS + JS الشامل (سطح المكتب + الهاتف) ===
# ============================================================
_GLOBAL_STYLE = """
<style>
/* ===================== قاعدة مشتركة ===================== */
...
.stSidebar input,
.stSidebar select,
.stSidebar textarea { color: #000000 !important; }
...
@media (min-width: 769px) {
    ...
    /* إخفاء شريط streamlit الافتراضي في سطح المكتب */
    .stApp header,
    .stApp .stToolbar,
    .stApp .stHeader,
    ...
    /* زر collapsedControl في سطح المكتب: أسفل اليسار */
...
@media (max-width: 768px) {
    ...
    .main .block-container {
        ...
        margin-top: 70px !important;   /* مسافة لأجل الـ header الثابت */
        max-width: 100% !important;
        border-radius: 0 !important;
        box-shadow: none !important;
    }

    /* Header ثابت مضغوط للهاتف */
    #gp-fixed-header {
        position: fixed;
        top: 0; left: 0; right: 0;
        width: 100%;
    ...
    #gp-fixed-header .sub   { display:none; }
    #gp-fixed-header .inst  { display:none; }
    #gp-fixed-header .dev   { font-size:0.7rem; color:#dbeafe; }

    /* إخفاء زر collapsedControl الأصلي على الهاتف - سنستخدم زر Menu الخاص بنا */
    button[data-testid="collapsedControl"] {
        display: none !important;
    }
...
.gp-kpi-value { font-size:1.8rem; font-weight:800; margin:.1rem 0 0; color:#0066cc; text-shadow:0 1px 2px rgba(0,102,204,.2); }
.gp-pill {
...
div.stButton > button[kind="primary"]:hover {
    ...
    transform: translateY(-2px) !important; box-shadow: 0 6px 20px rgba(0,102,204,.4) !important;
}

/* ===== Sidebar تصميم ===== */
[data-testid="stSidebar"] { background-color:#1e3a8a !important; padding-top:20px !important; }
[data-testid="stSidebar"] .st-emotion-cache-16txtl3 { padding-top:0rem !important; }
...
[data-testid="stSidebar"] div[aria-checked="true"] > label { background-color:transparent !important; color:#FFFFFF !important; font-weight:600 !important; }
...
[data-testid="stSidebar"] button:not([kind="primary"]):hover { background-color:rgba(255,255,255,0.1) !important; color:#FFFFFF !important; }
...
section[data-testid="stSidebar"] nav ul li a { font-size:18px !important; font-weight:600 !important; color:#FFFFFF !important; }
section[data-testid="stSidebar"] nav ul li a[aria-current="page"] { background-color:#1E88E5 !important; border-radius:8px !important; }
...
# ===================== JavaScript =====================
# يتولى:
# 1. مزامنة حالة sidebar_open مع الزر الأصلي لـ Streamlit
# 2. مراقبة تغيير حالة Sidebar من أي مصدر (زرنا أو زر Streamlit)
_SYNC_JS = """
<script>
(function () {
    // ==========================================
    // مزامنة الحالة بين زر Menu الخاص بنا وزر Streamlit الأصلي
    // ==========================================
    ...
    function isSidebarVisible() {
        ...
        return style.display !== 'none' && style.visibility !== 'hidden' && sb.offsetWidth > 0;
    }

    // عند الضغط على زر Streamlit الأصلي (collapsedControl)، نُشعر Streamlit لتحديث session_state
    function watchStreamlitBtn() {
        var stBtn = document.querySelector('button[data-testid="collapsedControl"]');
        if (!stBtn) { setTimeout(watchStreamlitBtn, 500); return; }
        stBtn.addEventListener('click', function () {
            // بعد انتهاء أنيميشن Streamlit، نبحث عن زر Menu الخاص بنا ونضغطه
            setTimeout(function () {
                // نبحث عن زر ☰ Menu ضمن العنصر floating-menu-btn
                var menuDiv = document.getElementById('floating-menu-btn');
                if (menuDiv) {
                    var menuBtn = menuDiv.querySelector('button');
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function() { setTimeout(watchStreamlitBtn, 800); });
    } else {
        setTimeout(watchStreamlitBtn, 800);
    }
})();
</script>
"""
...
# ============================================================
def sidebar_toggle():
    # تطبيق CSS الشامل أولاً
    st.markdown(_GLOBAL_STYLE, unsafe_allow_html=True)

    # إخفاء Sidebar بـ CSS إذا كانت مغلقة
